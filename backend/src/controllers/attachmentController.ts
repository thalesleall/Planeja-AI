...
import { generateThumbnail, getImageMetadata, isImage } from '../utils/imageProcessor';
...
export class AttachmentController {
    ...
    try {
      if (!attachmentModel) {
        return res.status(503).json({ 
          success: false, 
          message: 'MongoDB não disponível. Anexos temporariamente indisponíveis.' 
        });
      }
      ...
      if (error || !task || (task.to_do_list as any).owner_id !== parseInt(req.user.id)) {
        // Limpar arquivos já enviados
        files.forEach(file => fs.unlinkSync(file.path));
        return res.status(404).json({ success: false, message: 'Task não encontrada ou sem permissão' });
      }
  ...
  static async list(req: Request, res: Response) {
    try {
      ...
      if (!task || (task.to_do_list as any).owner_id !== parseInt(req.user.id)) {
  ...
  static async delete(req: Request, res: Response) {
    try {
      if (!attachmentModel) {
        return res.status(503).json({ success: false, message: 'MongoDB não disponível' });
      }
      ...
      if (!task || (task.to_do_list as any).owner_id !== parseInt(req.user.id)) {
        return res.status(403).json({ success: false, message: 'Sem permissão para deletar este anexo' });
      }

      // Deletar arquivos físicos
      const originalPath = path.join(__dirname, '../../uploads/tasks', taskId.toString(), 'original', attachment.filename);
      const thumbPath = path.join(__dirname, '../../uploads/tasks', taskId.toString(), 'thumbnails', attachment.filename);
  ...
  static async serve(req: Request, res: Response) {
    try {
      ...
      const filePath = path.join(__dirname, '../../uploads/tasks', taskId, 'original', filename);
  ...
  static async serveThumbnail(req: Request, res: Response) {
    try {
      const { taskId, filename } = req.params;
      const thumbPath = path.join(__dirname, '../../uploads/tasks', taskId, 'thumbnails', filename);

      if (!fs.existsSync(thumbPath)) {
        // Se não houver thumbnail, retornar original
        const originalPath = path.join(__dirname, '../../uploads/tasks', taskId, 'original', filename);
        if (fs.existsSync(originalPath)) {
          return res.sendFile(originalPath);
        }
        return res.status(404).json({ success: false, message: 'Thumbnail não encontrado' });
      }
