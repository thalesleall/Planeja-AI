...
export function startRefreshTokenCleanup(intervalMs = 1000 * 60 * 60) {
  ...
  async function cleanup() {
    try {
      ...
      if (!ok) {
        console.warn('refreshTokenCleanup: Supabase not reachable; skipping cleanup this run.');
        return;
      }
      ...
      if (err && err.message && err.message.includes('fetch failed')) {
        console.warn('refreshTokenCleanup: network fetch failed when calling Supabase. Is SUPABASE_URL reachable?');
      }
